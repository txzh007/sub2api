...
// duration 请求参数允许 1-15 秒；未指定时上游默认生成 8 秒。
// 当前视频按次计费，但仍归一化并记录时长，便于用量审计与后续切换计费策略。
const (
...
// NormalizeVideoBillingDurationSecondsOrDefault 归一化视频用量元数据中的时长：
// 未指定（<=0）按上游默认 8 秒记，超出上游允许区间按边界收敛。
