...
const apiKeyAuthSnapshotVersion = 25 // v25: per-key image bridge model + enforced group model_allowlist
...
func (s *APIKeyService) snapshotFromAPIKey(ctx context.Context, apiKey *APIKey) *APIKeyAuthSnapshot {
	...
	snapshot := &APIKeyAuthSnapshot{
		...
		ImageBridgeModel: apiKey.ImageBridgeModel,
		Name:             apiKey.Name,
...
func (s *APIKeyService) snapshotToAPIKey(key string, snapshot *APIKeyAuthSnapshot) *APIKey {
	...
	apiKey := &APIKey{
		...
		ImageBridgeModel: snapshot.ImageBridgeModel,
		Key:              key,
		Name:             snapshot.Name,
