...
type UpdateAPIKeyRequest struct {
	ImageBridgeModel       *string   `json:"image_bridge_model"`
	UpdateImageBridgeModel bool      `json:"-"`
	Name                   *string   `json:"name"`
	GroupID                *int64    `json:"group_id"`
...
type APIKeyService struct {
	imageBridgeAccounts       AccountRepository
...
func (s *APIKeyService) Create(ctx context.Context, userID int64, req CreateAPIKeyRequest) (*APIKey, error) {
	...
	var key string
	if req.ImageBridgeModel != nil {
		model := strings.TrimSpace(*req.ImageBridgeModel)
		if model != "" {
			if err := validateImageBridgePrimaryGroup(primaryGroup); err != nil {
				return nil, err
			}
			if _, _, err := s.ResolveImageBridge(ctx, userID, model); err != nil {
				return nil, err
			}
		}
		req.ImageBridgeModel = &model
	}
	...
	// 创建API Key记录
	apiKey := &APIKey{
		ImageBridgeModel: req.ImageBridgeModel,
		UserID:           userID,
		Key:              key,
...
func (s *APIKeyService) Update(ctx context.Context, id int64, userID int64, req UpdateAPIKeyRequest) (*APIKey, error) {
	...
	if req.UpdateImageBridgeModel || req.ImageBridgeModel != nil {
		if req.ImageBridgeModel != nil {
			model := strings.TrimSpace(*req.ImageBridgeModel)
			if model != "" {
				if err := validateImageBridgePrimaryGroup(apiKey.Group); err != nil {
					return nil, err
				}
				if _, _, err := s.ResolveImageBridge(ctx, userID, model); err != nil {
					return nil, err
		...
		apiKey.ImageBridgeModel = req.ImageBridgeModel
		fields.ImageBridgeModel = true
	} else if fields.GroupID && !KeySupportsImageBridge(apiKey) && apiKey.ImageBridgeModel != nil {
		// Changing the primary group must not leave an invisible bridge active.
		disabled := ""
		apiKey.ImageBridgeModel = &disabled
		fields.ImageBridgeModel = true
	}
