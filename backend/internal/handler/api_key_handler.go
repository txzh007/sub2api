...
type UpdateAPIKeyRequest struct {
	ImageBridgeModel imageBridgeModelField `json:"image_bridge_model"`
...
func (h *APIKeyHandler) Update(c *gin.Context) {
	...
	svcReq := service.UpdateAPIKeyRequest{
		ImageBridgeModel:       req.ImageBridgeModel.Value,
		UpdateImageBridgeModel: req.ImageBridgeModel.Present,
		IPWhitelist:            req.IPWhitelist,
		IPBlacklist:            req.IPBlacklist,
		Quota:                  req.Quota,
...
// Distinguish omission (leave unchanged), null (inherit), and empty (disable).
type imageBridgeModelField struct {
	Present bool
	Value   *string
}

func (f *imageBridgeModelField) UnmarshalJSON(data []byte) error {
	f.Present = true
	return json.Unmarshal(data, &f.Value)
}

func (h *APIKeyHandler) GetImageBridgeModels(c *gin.Context) {
	subject, ok := middleware2.GetAuthSubjectFromContext(c)
	if !ok {
		response.Unauthorized(c, "User not authenticated")
		return
	}
	models, err := h.apiKeyService.GetImageBridgeModels(c.Request.Context(), subject.UserID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
