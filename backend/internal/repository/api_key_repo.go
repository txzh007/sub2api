...
func (r *apiKeyRepository) Create(ctx context.Context, key *service.APIKey) error {
	builder := r.client.APIKey.Create().
		...
		SetNillableImageBridgeModel(key.ImageBridgeModel).
...
func (r *apiKeyRepository) Update(ctx context.Context, key *service.APIKey, fields service.APIKeyUpdateFields) error {
	...
	if fields.ImageBridgeModel {
		if key.ImageBridgeModel == nil {
			builder.ClearImageBridgeModel()
		} else {
			builder.SetImageBridgeModel(*key.ImageBridgeModel)
		}
	}
	if fields.Status {
...
func apiKeyEntityToService(m *dbent.APIKey) *service.APIKey {
	...
	out := &service.APIKey{
		...
		GroupID:          m.GroupID,
		ImageBridgeModel: m.ImageBridgeModel,
		Quota:            m.Quota,
