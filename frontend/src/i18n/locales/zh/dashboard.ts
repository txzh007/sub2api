export default {
  imageProviders: {
    title: '生图接口',
    description: '独立管理生图上游地址、密钥和模型',
    hint: '接口绑定到“生图”分组，供 OpenAI 密钥桥接使用。客户端统一使用 OpenAI Images 协议。',
    create: '添加生图接口',
    edit: '编辑生图接口',
    ...
    copyConfirm: '复制为生图账号',
    copying: '复制中…',
    copySuccess: '已复制为生图账号“{name}”，请检查后再启用。',
    accountType: '账号类型',
    accountTypeHint: '根据上游账号选择 OpenAI、Gemini 或 Grok / xAI；系统会按账号类型选择对应的生图路由。',
    models: '生图模型',
    fetchModels: '从远端获取模型',
    fetchingModels: '获取中…',
    fetchModelsHint: '从远端列表中勾选模型，保存后生效。编辑时自动使用已保存的密钥获取。',
    fetchToSelect: '填写接口地址和密钥，点击“从远端获取模型”后选择。',
    ...
    selectModelRequired: '请至少选择一个生图模型。',
    searchModels: '搜索远端模型',
    imageModelsOnly: '仅看生图相关名称',
    selectVisibleModels: '选中当前列表',
    remoteModelsCount: '显示 {count} / {total} 个远端模型',
    noMatchingModels: '没有匹配的模型，可取消名称筛选或调整搜索词。',
    noRemoteModels: '远端未返回模型，可重试或手动填写。',
    modelsHint: '每行一个模型；映射可写为 别名=上游模型。候选模型受“生图”分组模型列表设置限制。',
    invalidModels: '请填写具体模型名称和映射目标，不支持通配符。',
    keepKey: '留空保留原密钥',
    concurrency: '最大并发',
    groupUnavailable: '请先启用名为“生图”的分组，并开启图片生成权限。',
    enable: '启用',
    disable: '停用',
    disabled: '已停用',
    deleteTitle: '删除生图接口',
    deleteConfirm: '确定删除生图接口“{name}”吗？对应账号及其所有分组关联将一并删除。',
    deleteImpact: '若某个模型没有其他可用接口承接，选择该模型的密钥将无法继续生图。',
    deleting: '删除中…',
    deleted: '生图接口已删除',
    failed: '操作失败，请重试',
    saved: '生图接口已保存',
  ...
  keys: {
    ...
    groupLabel: '分组',
    imageBridgeModel: '生图桥接模型',
    imageBridgeDisabled: '不桥接',
    imageBridgeDefault: '跟随服务默认配置',
    imageBridgeHint: '文字使用主分组，图片使用独立生图池中的所选模型，并按生图分组计费。',
    imageBridgeEmpty: '暂无可用模型，请先启用生图分组并配置可调度的生图接口。',
    imageBridgeLoadFailed: '生图模型加载失败',
    imageBridgeRetry: '重试',
    imageBridgeUnavailable: '当前不可用',
    imageBridgeReasonNoProvider: '分组内没有提供该模型的生图接口',
    imageBridgeReasonAccountInactive: '提供该模型的接口未启用',
    imageBridgeReasonAccountUnschedulable: '提供该模型的接口已暂停调度',
    imageBridgeReasonWrongPurpose: '账号尚未标记为生图接口',
    imageBridgeReasonNotAllowed: '该模型不在生图分组白名单中',
    imageBridgeReasonTemporary: '提供方当前暂时不可用',
    selectGroup: '选择分组',
    statusLabel: '状态',
    selectStatus: '选择状态',
    saving: '保存中...',
