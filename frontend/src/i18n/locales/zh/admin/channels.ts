export default {
    ...
    modelPricing: {
      title: '模型价格库',
      description: '集中维护全部模型的目录价与本地覆盖规则，修改后立即生效',
      refreshRemote: '同步线上目录',
      addRule: '新增价格规则',
      enabledGroupModels: '当前分组模型',
      groupCoverage: '来自 {groups} 个启用分组；完整目录 {catalog} 项',
      localOverrides: '本地覆盖',
      ...
      updatedAt: '最近更新',
      prefixRule: '前缀规则',
      inheritedRule: '继承 {rule}',
      deprecated: '已弃用',
      deprecationDate: '弃用日期：{date}',
      usedByGroups: '当前分组：{groups}',
      unknownProvider: '未指定供应商',
      sourceOverride: '本地覆盖',
      sourceCatalog: '目录价',
      sourceInherited: '继承规则',
      sourceMissing: '未定价',
      restoreCatalog: '删除覆盖并恢复目录价',
      restoreAction: '删除覆盖',
      restoreConfirm: '确定删除“{model}”的本地覆盖吗？删除后会立即恢复线上目录或备用文件中的价格。',
      editRule: '编辑价格规则',
      editorHint: 'OpenAI、Gemini、Claude、Grok 使用 USD；其他模型使用 RMB。人民币价格按录入数值直接计价，不做汇率换算。',
      modelOrPrefix: '模型名或前缀规则',
      mode: '模式',
      provider: '供应商',
      ...
      filters: {
        ...
        specificGroup: '分组：{group}',
        fullCatalog: '全部价格目录',
        unusedCatalog: '未被当前分组使用',
        allSources: '全部来源',
        allModes: '全部模式'
      },
      columns: {
      ...
      invalidPrice: '价格必须是大于或等于 0 的数字',
      priceRequired: '至少填写一个价格字段',
      modelRequired: '请输入模型名或前缀规则',
      loadFailed: '加载模型价格库失败',
      groupModelsLoadPartial: '有 {count} 个分组的模型列表加载失败，当前结果可能不完整',
      refreshSuccess: '线上价格目录已同步',
      refreshFailed: '同步线上价格目录失败',
      saveSuccess: '价格规则已保存并生效',
      saveFailed: '保存价格规则失败',
      restoreSuccess: '已删除覆盖并恢复目录价',
      restoreFailed: '恢复目录价失败'
    },
